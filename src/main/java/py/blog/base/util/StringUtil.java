...
/**
 * import sun.io.ByteToCharConverter; import sun.io.CharToByteConverter;
 **/
...
public class StringUtil {
	...
	*</code>
	...
	/*
	 * public static String toHtml(String s) { //显示 s = Replace(s, "&", "&amp;"); s = Replace(s, "\\", "&#92;"); s = Replace(s, "\"", "&quot;"); s = Replace(s, "<", "&lt;"); s = Replace(s, ">", "&gt;"); s = Replace(s,
	 * "\t", "    "); s = Replace(s, "\r\n", "\n"); // s = Replace(s, "\n", "<br>"); s = Replace(s, " ", "&nbsp;"); // s = Replace(s, "'", "&#39;"); // s = Replace(s, "%", "%"); return s; } public static String
	 * unHtml(String s) { s = Replace(s, "&lt;", "<"); s = Replace(s, "&gt;", ">"); s = Replace(s, "    ", "\t"); s = Replace(s, "\n", "\r\n"); s = Replace(s, "<br>", "\n"); s = Replace(s, "&nbsp;", " "); s = Replace(s,
	 * "&amp;", "&"); s = Replace(s, "&#39;", "'"); s = Replace(s, "&#92;", "\\"); s = Replace(s, "％", "%"); return s; }
	 */
	...
	/**
	 * Escapes a <code>String</code> according the JavaScript string literal escaping rules. The resulting string will not be quoted.
	 * <p>
	 * It escapes both <tt>'</tt> and <tt>"</tt>. In additional it escapes <tt>></tt> as <tt>\></tt> (to avoid <tt>&lt;/script></tt>). Furthermore, all characters under UCS code point 0x20, that has no dedicated escape
	 * sequence in JavaScript language, will be replaced with hexadecimal escape (<tt>\x<i>XX</i></tt>).
	 */
