...
public class ValidateUtil {
     ...
     *
     * @param pstrTarget 検査する文字列
     * @param pstrForbidden 禁則文字群(省略時(nullまたはサイズ0の文字列))
     * @return 検査結果
     * <table>
     * <tr>
     * <td>\u0000</td>
     ...
     * <br>
     * システムで規定されている禁則文字群　　<font face="ＭＳ ゴシック"><b>!"#$%&'()=~|\@[+;:]<>,._</b></font>
     *
     * @param pstrTarget 検査する文字列
     * @return 検査結果
     * <table>
     ...
     *
     * @param pstrTarget 検査する文字列
     * @param pintIntLen 整数部の最小桁数（検査をしないときは「-1」を指定する。整数部の桁数の判定は「最小桁数以上」となる。）
     * @param pintDecLen 小数部の最小桁数（検査をしないときは「-1」を指定する。小数部の桁数の判定は「最小桁数以上」となる。）
     * @return 検査結果
     * <table>
     * <tr>
     * <td align="right">0</td>
     ...
     *
     * @param pstrTarget 検査する文字列
     * @param pintIntLen 整数部の最大桁数（検査をしないときは「-1」を指定する。整数部の桁数の判定は「最大桁数以下」となる。）
     * @param pintDecLen 小数部の最大桁数（検査をしないときは「-1」を指定する。小数部の桁数の判定は「最大桁数以下」となる。）
     * @return 検査結果
     * <table>
     * <tr>
     * <td align="right">0</td>
     ...
     *
     * @param pstrTarget 検査する文字列
     * @param pintSignPos 符号の位置（符号が必要な場合、どこに書かれるか）
     * <table>
     * <tr>
     * <td align="right">1</td>
     * <td>先頭にあること</td>
    ...
    public static int chkNumeric(String pstrTarget, int pintSignPos) {
        ...
        if(intCheckSign != 0){ // 符号があったとき
            if(intCheckSign == 6 || intCheckSign == 5){ // 符号が先頭、末尾以外にあった or 符号が2つ以上あった
                return -1;
            }else{
     ...
     * 引数のlineSeparateFlagがtrueの場合は、文字列中に改行文字が含まれていても入力検査エラーとならないが、 falseの場合は、改行文字が含まれていると入力検査エラーとなる。
     *
     *
     * @param value 文字列
     * @param lineSeparateFlag 改行文字入力許可フラグ（true:改行コード入力可能／false:改行コード入力不可能）
     * @return 検査結果
     * <table>
     * <tr>
     * <td>true</td>
     ...
     * 元の文字列がnullのときは、""を返す。
     *
     * @param pstrString 元の文字列
     * @param pintSign 符号の位置
     * <table>
     * <tr>
     * <td align="right">1</td>
     * <td>先頭に+がある。</td>
    ...
    public static int chkDecimalFormat(String aTarget) {
        ...
        } // end of 'for(char c = ite.first(); c != CharacterIterator.DONE; c = ite.next()){'
    ...
    public static boolean chkDate(String year, String month, String day) {
        //lengthのチェック
        if(year == null || month == null || day == null || year.length() != 4 || month.length() > 2 || day.length() > 2){
            return false;
        }
     ...
     *
     * @param dateValue yyyyMMdd型の日付文字列
     ...
     *
     * @param dateValue yyyyMMdd型の年月文字列
     ...
     * UTF-8環境では全角は必ずしも(というかほとんど)2byteにはならず3byteになる。よって、 よって、バイト数での厳密なチェックは行わない。文字数のチェックとなる。<br>
     * min と max が双方nullの場合はNullPointerExceptionを投げる
     *
     * @param value 検査対照の文字列(nullを指定した場合は検査対象とないためtrueを返却)
     * @param min 最小桁数(nullを指定した場合は最大桁数のチェックだけをして最小桁数はチェックしない)
     * @param max 最大桁数(nullを指定した場合は最小桁数のチェックだけをして最大桁数はチェックしない)
     ...
     *
     * @param baseDate yyyyMMdd形式またはyyyy/MM/dd形式の日付文字列
     * @param date yyyyMMdd形式またはyyyy/MM/dd形式の日付文字列
     * @param notEqual true：等しい値をエラーとする、false：等しい値を正常とする
