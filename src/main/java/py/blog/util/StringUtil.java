...
public class StringUtil {
	...
	public static String toRegularCase(String str) {
		...
		CharacterIterator ite = new StringCharacterIterator(str.substring(1,
				str.length()));
	 ...
	 * <UL>
	 * <LI>
	 * 切り出す対象範囲の開始位置が全角文字を分割する位置になるとき、切り出す対象範囲の開始位置は該当する文字の<b>次の文字に補正</b>される。
	 * <LI>
	 * 切り出す対象範囲の終了位置が全角文字を分割する位置になるとき、切り出す対象範囲の終了位置は該当する文字の<b>前の文字に補正</b>される。
	 * </UL>
	 * <br>
	...
	public static int countLengthByBuildByte(String aTarget) {
		return StringUtil.countLengthByBuildByte(aTarget,
				CommonProperty.getEncode());
	}
	...
	public static int intValueFromDate(Date aDate) {
	...
	public static String strValueFromDate(Date aDate) {
	...
	public static String[] strValueArrayFromDate(Date aDate) {
	...
	public static int[] intValueArrayFromDate(Date aDate) {
	...
	public static String replaceAllString(String pstrString, String pstrOldStr,
			String pstrNewStr) {
	...
	public static String deleteStringFromString(String pstrString,
			String pstrDelStr) {
	 ...
	 * 文前後にある空行→除去、文中の空行→残す場合　StringUtil.trim(StringUtil.trimMulti(value,false))
	 * と使用する。<br>
	 ...
	 * 小数の丸め方法は {@link RoundingMode#HALF_EVEN} に従います。
	 * 
	 * @param val
	 *            変換する値
	 * @param srcFormat
	 ...
	 * @return 変換後の値
	 */
	public static String convertDecimal(String val, String srcFormat,
			String outFormat) {
	 ...
	 * @return 変換後の値
	 */
	public static String convertDecimalRnd(String val, String srcFormat,
			String outFormat, RoundingMode roundingMode) {
		if (val == null) {
			return val;
		}
		...
		if (srcFormat != null) {
			...
			decFormat.applyPattern(matcher.group(1) + decFormat.toPattern()
					+ matcher.group(3));
		}
		...
		if (num instanceof BigDecimal && roundingMode != null) {
			...
			if (roundingMode != null) {
				...
				mc = new MathContext(setPrecision);
			}
	...
	// //////////////////////////////////////////////////////////////////////////////
	private static String pad(String inStr, int byteLen, String strPad,
			boolean rightPad) {
		...
		if (inLen > byteLen) { // 元の文字列の長さが、長さの上限を超えているとき。
			if (rightPad) {
				...
				return StringUtil.substringByByte(inStr, (targetLen * -1),
						inLen); // 左側に文字を連結しようとしたとき。
			}
